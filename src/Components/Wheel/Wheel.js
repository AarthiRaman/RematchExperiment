...
export default function Wheel({ form, wheel }) {
    ...
    const colorWheel = ['#c51b8a', '#fa9fb5', '#fde0dd'];
        ...
        {entryArrays.join(',')}
           {
            entryArrays.map((val, i) => <g>
                <path id={`arc${i}`} d={getSectorPath(outerDiameter / 2, outerDiameter / 2, outerDiameter, calculateAngle(entryArrays.length, i).start, calculateAngle(entryArrays.length, i).end)} fill={colorWheel[i % 3]}/>
                <text>
                <textPath startOffset={"60%"} side="right" href={`#arc${i}`}>
                  {val}
                </textPath>
              </text>
              </g>
